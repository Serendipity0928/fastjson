...
public class SymbolTable {
    ...
    public SymbolTable(int tableSize){
        this.indexMask = tableSize - 1;     // tableSize默认4096(0x1000), 因此indexMask为0xFFF
    ...
    public String addSymbol(String buffer, int offset, int len, int hash, boolean replace) {
        // TODO: 2022/11/25 具体怎么算？
        final int bucket = hash & indexMask;    // 根据hash值和0xFFF相与
        ...
        symbol = symbol.intern();   // 调用String.intern()方法
        symbols[bucket] = symbol;
        return symbol;
    }

    private static String subString(String src, int offset, int len) {
        // TODO: 2022/11/25 为什么不直接使用String的截取方法呢？ 
